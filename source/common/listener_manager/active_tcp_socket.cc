...
void ActiveTcpSocket::continueFilterChain(bool success) {
  if (success) {
    ...
    if (no_error) {
      ...
      // to the network filter chain matching, which can block as well. So we have
      // to keep the ActiveTcpSocket around until matching is done. By returning
      // here we are delegating to the newConnection to decide when to release the
      // ActiveTcpSocket.
      return;
    } else {
...
void ActiveTcpSocket::newConnection() {
  ...
  if (new_listener.has_value()) {
    ...
    accept_filters_.clear();
    // Procceed to finding the network filter chain and creating a new connection on this listener.
    matching_ = true;
