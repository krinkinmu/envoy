...
int32_t Builder::hostFunctionEntry(
        uint64_t cookie, uint8_t *data, uint64_t size) {
    Cookie* f = reinterpret_cast<Cookie*>(cookie);
    return f->run(absl::Span<uint8_t>(data, size));
}

void Builder::registerFunction(
    ...
    ::sandbox_builder_register_host_function(
        impl_, id, name.c_str(), &hostFunctionEntry);
...
absl::StatusOr<std::unique_ptr<Sandbox>> Builder::build() {
    ...
    if (result != 0) {
       return absl::InternalError(
           "hyperlight wasm failed to construct proto sandbox"); 
    }
